...
class rosbag_generator
{
    ...
    /* ros subscriber */
    ros::Publisher pub_WLSENU, pub_FGOENU, pub_global_path, pubGNSSRaw, pubStationGNSSRaw, pubRoverGNSSRaw, pubDopplerVel, pubAdrVrel;
    ...
    std::unique_ptr<message_filters::Subscriber<nav_msgs::Odometry>> doppler_sub, adr_sub;
    std::unique_ptr<message_filters::Subscriber<nlosExclusion::GNSS_Raw_Array>> rover_gnss_raw_array_sub, station_gnss_raw_array_sub;
    std::unique_ptr<message_filters::TimeSynchronizer<nlosExclusion::GNSS_Raw_Array, nlosExclusion::GNSS_Raw_Array, nlosExclusion::GNSS_Raw_Array,
        nav_msgs::Odometry, nav_msgs::Odometry>> syncdoppler2GNSSRaw;
...
public:
    ...
    {      
        ...
        adr_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(nh, "/gnss_preprocessor_node/GNSSAdrVelRov1", 10000));
        /* GNSS measurements from station (reference end) */
        rover_gnss_raw_array_sub.reset(new message_filters::Subscriber<nlosExclusion::GNSS_Raw_Array>(nh, "/gnss_preprocessor_node/GNSSPsrCarRover1", 10000)); // measurements from station
        ...
        syncdoppler2GNSSRaw.reset(new message_filters::TimeSynchronizer<nlosExclusion::GNSS_Raw_Array, 
            nlosExclusion::GNSS_Raw_Array, nlosExclusion::GNSS_Raw_Array,
            nav_msgs::Odometry, nav_msgs::Odometry>(*gnss_raw_array_sub, 
            *rover_gnss_raw_array_sub, *station_gnss_raw_array_sub, *doppler_sub, *adr_sub, 32));
        syncdoppler2GNSSRaw->registerCallback(boost::bind(&rosbag_generator::gnssraw_doppler_msg_callback,
            this, _1, _2, _3, _4, _5));
        ...
        pubRoverGNSSRaw = nh.advertise<nlosExclusion::GNSS_Raw_Array>("/rosbag_generator_node/GNSSPsrCarRover1", 100); // 
        ...
        pubAdrVrel = nh.advertise<nav_msgs::Odometry>("/rosbag_generator_node/GNSSAdrVelRov1", 100); // 
        ...
        bag.open("/home/alexey/rtklib.msg", rosbag::bagmode::Write);
    }
   ...
   * @return void
   @ 
   */
   void gnssraw_doppler_msg_callback(const nlosExclusion::GNSS_Raw_ArrayConstPtr& gnss_msg, 
        ...
        const nav_msgs::OdometryConstPtr& adr_msg)
    {
        ...
        if(checkValidEpoch(time_frame) && m_GNSS_Tools.checkRepeating(*gnss_msg))
        {
            ...
            {
                bag.write("doppler", ros::Time::now(), *doppler_msg);
                bag.write("adrvel", ros::Time::now(), *adr_msg);
                bag.write("raw", ros::Time::now(), *gnss_msg);
                bag.write("rover", ros::Time::now(), *rover_gnss_msg);
                bag.write("station", ros::Time::now(), *station_gnss_msg);

                if (gnss_frame % 100 == 0)
                {
                    LOG(INFO) << "message written "<< gnss_frame << std::endl;
                }
    ...
    ~rosbag_generator()
    {
        LOG(INFO) << "closing bag .. " << gnss_frame << " messages written" << std::endl;
        bag.close();
        LOG(INFO) << "closing bag .. done " << std::endl;
    }
