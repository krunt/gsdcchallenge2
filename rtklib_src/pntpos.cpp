...
{
    prcopt_t opt_=*opt;
    double *rs,*dts,*var,*azel_,*resp, freq;
    ...
    double *posvar;
    {
        ...
        v=mat(n+4,1); H=mat(NX,n+4); posvar=mat(n+4,1);
        
        for (i=0;i<3;i++) x[i]=sol->rr[i];

        rescode(i,obs,n,rs,dts,var,svh,nav,x,&opt_,ssat,v,H,posvar,azel_,vsat,resp,&ns);
    }
    ...
    {
        if (!(sys=satsys(obs[s_i].sat,NULL))) continue;
        if (s_i<n-1&&obs[s_i].sat==obs[s_i+1].sat) {
            // trace(2,"duplicated obs data %s sat=%d\n",time_str(time,3),sat);
            s_i++;
            continue;
        }
        if (satexclude(obs[s_i].sat,var[s_i],svh[s_i],&opt_)) continue;

        if (!snrmask(&obs[s_i],azel_+s_i*2,&opt_)) continue;
        ...
        if ((freq=sat2freq(obs[s_i].sat,obs[s_i].code[0],nav))==0.0) continue;
        dion*=SQR(FREQL1/freq);
        vion*=SQR(FREQL1/freq);
        ...
        */
        ...
        gnss_raw.visable = ssat[obs[s_i].sat-1].slip[0];

        gnss_raw.pos_var = posvar[s_i];
    ...
    if(1) // use the snr and ele to model the uncertainty of doppler
    {
        ...
        odometry.twist.covariance[0] = sol->qv[0]; // qv?
        odometry.twist.covariance[1] = sol->qv[1]; // qv?
        odometry.twist.covariance[2] = sol->qv[2]; // qv?
