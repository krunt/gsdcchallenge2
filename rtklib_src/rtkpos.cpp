...
static int ddres(rtk_t *rtk, const nav_t *nav, const obsd_t *obs, double dt, const double *x,
                 ...
                 int ns, double *v, double *H, double *R, int *vflg, double *satvar)
{
    ...
    int b1;
    for (k=0,b1=0;b1<b;k+=nb[b1++]) {  /* loop through each system */
        double sum1 = 0;
        for (i=0;i<nb[b1];i++) {
            sum1+=R[k+i+(k+i)*nv];
        }
        satvar[b1] = sum1 / (nb[b1]+1e-7);
    }
                  ...
                  int nv, double thres)
{
    ...
    for (i=0;i<nv;i++) {
        ...
        type=(vflg[i]>> 4)&0xF;
        freq=vflg[i]&0xF;
        stype=(char*)(type==0?"L":(type==1?"P":"C"));
                  ...
                  const nav_t *nav)
{
    ...
    int i,j,f=0,n=nu+nr,ns,ny,nv,sat[MAXSAT],iu[MAXSAT],ir[MAXSAT],niter;
    ...
    for (i=0;i<niter;i++) {
        ...
        if ((nv=ddres(rtk,nav,obs,dt,xp,Pp,sat,y,e,azel,freq,iu,ir,ns,v,H,R,vflg,satvar))<4) {
    ...
    for (int rover = 0; rover < 2; ++rover)
    {
        ...
        int* rix = rover == 0 ? iu : ir;
        for(int is=0; is<ns; is++) {

            for (f=0;f<nf;f++) {

                int sys;
                if (!(sys=satsys(obs[rix[is]].sat,NULL))) continue;

                if (satexclude(obs[rix[is]].sat,var[rix[is]],svh[rix[is]],opt)) continue;

                if (!snrmask(&obs[rix[is]],azel+rix[is]*2,opt)) continue;
...

                {
                    ...
                    double freq;
                    if ((freq=sat2freq(obs[rix[is]].sat,obs[rix[is]].code[f],nav))==0.0) continue;
                    ...
                    for (int m=0;m<6;++m) {
                        if (!test_sys(sysi,m) || sysi==SYS_SBS) continue;
                        gnss_raw.pos_var = satvar[m*2];
                        gnss_raw.phase_var = satvar[m*2+1];
                        break;
                    }
    ...
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,var,svh,nav,xp,opt,0,y,e,azel,freq)) {
        ...
        nv=ddres(rtk,nav,obs,dt,xp,Pp,sat,y,e,azel,freq,iu,ir,ns,v,NULL,R,vflg,satvar);
...

    {
        ...
        odometry.twist.covariance[0] = rtk->sol.qv[0]; // qv?
